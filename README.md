...
## 검증 메타데이터와 claim 앵커 이관

`verified_by`에는 과거 검증자 ID와 대조 방법 설명이 혼재하므로 별도 이관기로 구조화한다. 기본은 dry-run이며 현재 corpus에서 확인한 exact mapping만 적용하고 기존 `verified_by`, `last_verified`, evidence 날짜는 바꾸지 않는다. unknown 값, 기존 구조화 metadata와의 충돌, frontmatter parse 오류가 하나라도 있으면 `--write`는 전체 쓰기를 중단한다. 템플릿의 빈 `verification` 객체는 아직 채우지 않은 placeholder로 처리한다.

```text
python scripts/migrate_verification_metadata.py --output build/verification-plan.json
python scripts/migrate_verification_metadata.py --write --output build/verification-write.json
```

`official_source_review`는 검토 범위를 별도 이름으로 남기지 않은 공식 자료 검토를 뜻한다. `source_cross_check`는 `note`에 명시한 자료와 문서 내용을 대조했음을 뜻하며, 명명된 자료는 하나일 수도 여러 개일 수도 있다. 새 검증자는 `schemas/verifiers.json`에 안정 ID를 등록하고 구조화 metadata를 사용한다. `status: verified`는 비어 있지 않은 구조화 verifier ID 또는 호환용 legacy verifier ID가 필요하다. legacy 값이 남아 있는 문서는 구조화 값도 함께 있어야 하지만, 검증 이력이 없는 문서에서는 `verification`이 선택 필드다.

`evidence.supports`는 같은 문서 본문의 Obsidian block ID를 가리킨다. ID 형식은 소문자 영문으로 시작하는 영문·숫자·하이픈 1~64자이며, 문단·목록 항목·인용문 끝에 `^claim-id`로 기록한다. heading, 표, 코드 fence에는 claim marker를 두지 않으며 한 렌더링 블록에는 하나만 둔다. 위치를 자동 추측하면 법적 주장 범위가 바뀔 수 있으므로 계획 도구는 근거·본문 섹션과 미해소 여부만 보고한다.

```text
python scripts/plan_claim_anchors.py --output build/claim-anchor-plan.json
python scripts/lint_wiki.py --require-claim-anchors --fail-on high
```

전체 corpus의 앵커 검토가 끝날 때까지 `--require-claim-anchors`는 단계적 강제 옵션이며 CI 기본값에는 적용하지 않는다. 사이트 렌더러는 승인된 `^claim-id`를 HTML fragment로 변환하고 검색 인덱스에서는 실제 block marker만 제거한다. graph export는 claim을 `claim:<entity>#<claim>`, source를 `source:<source>`, evidence reference를 `evidence:<entity>#<evidence>` namespace로 내보내며 각 구성요소는 percent-encoding한다.

## v1.2 이관의 검증·review 정책
...
- verified Rule·Case·Law·Interpretation의 핵심 주장마다 source locator와 해소 가능한 본문 `^claim-id` 존재
